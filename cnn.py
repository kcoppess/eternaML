...
def setup_conv_layers(x_input, w_shapes, b_shapes, N):
    x = tf.reshape(x_input, [-1, 640, 640, 1])
    weights = []
...
class CNN():
    ...
    def construction(self, conv_w_shapes, conv_b_shapes, fc_w_shapes, fc_b_shapes, learning_rate):
        self.pairmap = tf.placeholder(tf.float32, shape=[None, 640*640], name='pairmap')
        self.loop = tf.placeholder(tf.float32, shape=[None,6], name='loop')
